...
public class PredictionEngineImpl implements PredictionEngine {
    ...
    private static final String TAG = PredictionEngineImpl.class.getSimpleName();
    private List<PredictionEngine.PredictionListener> predictionListeners;
    private PredictionProcessor processor;
    private Context context;

    public PredictionEngineImpl(Context context) {
...
//        processor = new EMPredictionProcessor(context);
    ...
    @Override
    public List<Prediction> processEventSynchronous(Event event) {
        if (event != null) {
            Log.d(TAG, event.toString() + " received.");
        } else {
            Log.d(TAG, "null event received. Ignore sending to prediction processor");
